 ...
 */
 ...
 //商品详情requestApi('mini/combination.Spell/pink_detail', 'GET', {id :that.data.id}).then((res) => {
  getSeckillDetail: function () {
    var that = this;
  
	requestApi('mini/seckill.index/seckill_detail', 'GET', {id: that.data.id }).then((res) => {
	  var storeInfo = res.data.data.storeInfo;
	  var title = storeInfo.title;
      console.log(res);
      that.setData({
        ["parameter.title"]: title.length > 10 ? title.substring(0, 10)+'...' : title,
        storeInfo: storeInfo,
        imgUrls: storeInfo.images
      });
