...
Page({
  ...
  combinationDetail:function(){
	...
	requestApi('mini/combination.Spell/pink_detail', 'GET', {id :that.data.id}).then((res) => {
		 that.setData({
        ["parameter.title"]: res.data.data.storeInfo.title.substring(0,16),
        imgUrls: res.data.data.storeInfo.images,
        userInfo: res.data.data.user,
        storeInfo: res.data.data.storeInfo,
        pink: res.data.data.pink,
        pindAll: res.data.data.pindAll,
        reply: res.data.data.reply,
        replyCount: res.data.data.replyCount,
        itemNew: res.data.data.pink_ok_list,
        pink_ok_sum: res.data.data.pink_ok_sum,
        replyChance: res.data.data.replyChance
      });
      that.setTime();
  ...
  */
  goCat: function () {
    ...
    if (this.data.productAttr.length && productSelect === undefined && this.data.isOpen == true) return app.Tips({ title: '请选择属性' });
	 ...
	 requestApi('mini/Cart/go_buy', 'GET', {
		productId: that.data.storeInfo.product_id,
        secKillId: 0,
        bargainId: 0,
        combinationId: that.data.id,
        cartNum: that.data.cart_num == undefined ? 1 : that.data.cart_num ,
