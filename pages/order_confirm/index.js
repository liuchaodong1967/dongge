...
Page({
  ...
  */
  getConfirm:function(){
    var that=this;
    requestApi('mini/order/confirm','GET',{ cartId: this.data.cartId }).then((res)=>{
	   ...
	   that.setData({
			...
			totalPrice: app.help().Add(parseFloat(res.data.priceGroup.vipPrice), parseFloat(res.data.priceGroup.storePostage)),
  ...
  */
  getCouponList:function(){
    var that=this;
	requestApi('mini/user.coupons/get_use_coupon_order','GET',{ totalPrice:this.data.totalPrice  }).then((res)=>{
		
		that.setData({ 'coupon.list': res.data.data, openType:1});
	});
  ...
  */
  getaddressInfo:function(){
    var that=this;
    var url = that.data.addressId ? 'mini/user.address/get_user_address' : 'mini/user.address/default_address_list';
    //console.log(url)
    requestApi(url, 'GET', { addressId: that.data.addressId }).then((res) => {
      //console.log(res)
      var address = res.data.data;
      res.data.is_default = parseInt(address.is_default);
      that.setData({ addressInfo: address || {}, addressId: address.id || 0, 'address.addressId': address.id || 0 });
    });
  },
  ...
  address:function(){
    ...
    console.log(this.data.pagesUrl);
  },
 ...
 //提交订单
  SubOrder:function(e){
	...
	requestApi('mini/order/create_order', 'POST', { key:that.data.orderKey,data:JSON.stringify(data) }).then((res) => {
			var status = res.data.status; 
			var orderId = res.data.orderId; 
			var jsConfig = res.data.wxConfig; 
			console.log(status);
			var goPages = '/pages/order_pay_status/index?order_id=' + orderId+'&msg='+res.data.msg;
      //console.log(goPages);
			switch (status){
				case 'ISPAY': 
					 wx.hideLoading();
					 ...
					 break;
				case 'CUNZAI':case 'GUOQI': 
					 wx.hideLoading();
					 return app.Tips({ title: res.data.msg}, { tab: 5, url: goPages });
					 break;
