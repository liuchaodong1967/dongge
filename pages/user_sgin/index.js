...
Page({
  ...
  */
  getSignSysteam:function(){  
	   var that=this;
		requestApi('mini/public_Api/sign_day_num', 'GET', { name:'sign_day_num'}).then((res) => {
			//console.log(res.data.data.sign_day_num);
			that.setData({ signSystemList: res.data.data.sign_day_num, day: that.Rp(res.data.data.sign_day_num.length)});
	   });
  },
  ...
  */
  getUserInfo:function(){
    var that=this;
	requestApi('mini/user.Personal/my_info', 'GET', {issign:1}).then((res) => {
			console.log(res.data.sign_num);
  ...
  */
  getSignList:function(){
    var that=this;
	requestApi('mini/user.Sign/get_sign_list', 'GET', {page:1,limit:3}).then((res) => {
  ...
  */
  goSign:function(e){
	...
	requestApi('mini/user.sign/click_sign', 'POST', {}).then((res) => {
		that.setData({
			active:true,
